...
const CotizacionBar = () => {
  ...
  useEffect(() => {
    const fetchCotizaciones = async () => {
      try {
        const [oficial, tarjeta, blue, cripto] = await Promise.all([
          fetch("https://dolarapi.com/v1/dolares/oficial").then((r) =>
            r.json(),
          ),
          ...
          fetch("https://dolarapi.com/v1/dolares/cripto").then((r) => r.json()),
        ]);
        setCotizaciones({ oficial, tarjeta, blue, cripto });
      } catch {
  ...
  const items = [
    {
      ...
      valor: fmt(cotizaciones.oficial?.venta),
      clase: "",
    },
    {
      nombre: "Dolar Tarjeta",
      valor: fmt(cotizaciones.tarjeta?.venta),
      clase: "",
    },
    {
      nombre: "Dolar Blue",
      valor: fmt(cotizaciones.blue?.venta),
      clase: "cotiz-hide-sm",
    },
    {
      nombre: "Dolar Cripto",
      valor: fmt(cotizaciones.cripto?.venta),
      clase: "cotiz-hide-xs",
    },
  ];

  return (
    <div className="cotiz-bar">
      <div className="cotiz-items">
        {items.map(({ nombre, valor, clase }) => (
          <div key={nombre} className={`cotiz-pill ${clase}`}>
            <span className="cotiz-nombre">{nombre}</span>
            <span className="cotiz-valor">{valor}</span>
          </div>
        ))}
      </div>
      <div className="cotiz-footer">
        <span className="cotiz-footer-line" />
        <span className="cotiz-footer-text">
          Cotizaciones Aproximadas en Pesos Argentinos{" "}
        </span>
        <span className="cotiz-footer-line" />
      </div>
    </div>
        ...
        {showResult && (
          <div className="q-results animate-in">
            {computed.map((r, i) => (
              <div
                key={i}
                className={`q-result-row ${
                  i > 0 ? "q-result-row--accent" : ""
                }`}
              >
